...
| 功能 | 狀態 | 說明 |
|------|------|------|
| **功能 1：遠端管理 Tier 1** | **已交付** | Phase 1–6：連線、UI gating、狀態輪詢、文件與發布（v1.3.3） |
| **功能 1：遠端管理 Tier 2** | **已交付** | Phase 1–4：線上地圖、REST 設定唯讀、手動 unban、文件與發布（v1.4.0） |
| 功能 1 Tier 3+ | 未實作 | SSH/SFTP、banlist 列舉、INI 寫入等 |
...
palserver-GUI 過去假設每個伺服器實例都在**本機 Windows** 上。v1.3.0 起可建立**遠端連線**實例（metadata-only），透過 REST 管理已運行於 VPS 的 1.0 專服；本機 spawn、Steam 更新、Mod 管理等仍僅適用本機實例。
...
| 項目 | 位置 | 說明 |
|------|------|------|
| 遠端建立 UI | `CreateRemoteServerAlert.tsx` | **已實作**：連線測試 + 建立 |
| 首頁入口 | `Home.tsx` | **已啟用**「建立遠端連接」 |
| 遠端 badge | `ServerInstance.tsx` | **已實作** |
...
| 遠端編輯連線 | `EditServerAlert` | **已實作**（v1.3.1） |
| 遠端 Online 狀態 | `useServerOnlineStatus` | **已實作**（v1.3.2，REST 輪詢） |
| 遠端封禁名單限制說明 | 玩家頁 | **已實作**（v1.3.2） |
...
#### Tier 2 — 遠端設定與地圖（v1.4.0 已交付）

- [x] 線上地圖 proxy 支援遠端 REST host（Phase 1）
- [x] 官方 `GET /v1/api/settings` 遠端唯讀檢視（Phase 2）
- [x] 手動解除封禁 `POST /v1/api/unban`（Phase 3）
- [x] 文件、E2E、發布（Phase 4）

**Tier 2 不包含**：遠端 INI 寫入、banlist 完整列舉、SSH/SFTP（見 Tier 3）。
...
- [x] 首頁可開啟「建立遠端連接」，填寫必要欄位後可建立實例
- [x] 建立前連線測試失敗時有明確錯誤（埠不通、密碼錯、REST 未啟用）
- [x] 遠端實例出現在列表，視覺上可與本機實例區分（badge / 圖示）
- [x] 對遠端實例可：列出玩家、踢人、封禁、廣播、存檔、關機
- [x] 對遠端實例不可：本機 spawn 啟動、Steam 更新、Mod 管理（按鈕隱藏或 disabled + tooltip 說明）
- [x] 本機實例行為與 v1.2.0 **完全一致**（回歸測試）
- [x] README / FAQ 新增遠端管理章節（埠、安全、限制）
- [x] 五語系翻譯鍵補齊（`CreateRemoteServer` 等已存在，需補錯誤訊息與限制說明）
...
- **功能 5** 對遠端實例：Tier 2 已支援 REST 設定**唯讀**檢視；寫入仍須遠端 INI 或 Tier 3。
...
| 順序 | 功能 | 理由 | 建議版本 |
|------|------|------|----------|
| 1 | 功能 1 Phase 4（UI gating） | 完善遠端 UX，避免誤操作本機功能 | v1.3.1 |
| 2 | 功能 2 Phase A | 減少 1.0 更新事故 | v1.3.1 / v1.4.0 |
| 3 | 功能 5 Phase A | 設定匯入／匯出 | v1.3.1 / v1.4.0 |
| 4 | 功能 5 Phase B | 持續補鍵 | v1.3.x |
| 5 | 功能 2 Phase B | 需維護 Mod 清單 | 待定 |
| 6 | 功能 1 Tier 2 | 遠端地圖、設定唯讀、unban UX | **v1.4.0（已交付）** |
| 7 | 功能 1 Tier 3+ | SSH/SFTP、檔案級遠端 | v2.0+ |
...
| 2026-07-13 | 1.2 | v1.3.0 實作狀態摘要；功能 1 Phase 1–3 標記為已交付 |
