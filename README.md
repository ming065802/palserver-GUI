...
- **遠端伺服器管理（v1.4.0）**：透過 REST API 連線管理 VPS／遠端主機上已運行的 1.0 專服；Tier 1 日常維運 + Tier 2 線上地圖、設定唯讀、手動解封。
...
| **遠端連接（v1.4.0）** | 首頁右鍵建立遠端連接；支援線上地圖、世界設定唯讀、手動解封 |
...
| 世界設定 INI 編輯 | Tier 2 僅提供 REST 唯讀檢視；寫入須於遠端主機編輯 INI |
| Mod 管理 | 無遠端檔案存取 |
| 伺服器日誌 | 無遠端程序輸出 |
| 效能監控 | 僅本機程序 |
| 複製實例 | 遠端為 metadata-only |
| 封禁名單檔案 | 無法列舉遠端 `banlist.txt`；可 REST 封禁線上玩家或手動輸入 ID 解封 |
...
v1.4.0 完成 **遠端伺服器管理 Tier 2**（Phase 1–4），在 Tier 1 REST 維運基礎上，**不引入 SSH/SFTP** 即可使用下列能力：

#### Tier 2 新增能力

| 功能 | 說明 |
|------|------|
| 線上地圖 | 伺服器管理 → 線上地圖；本機 proxy 轉發遠端 REST `/players`、`/info` |
| 世界設定（唯讀） | 右側「世界設定」或路由進入；資料來源 `GET /v1/api/settings`，可重新整理 |
| 手動解除封禁 | 玩家頁輸入已知 Steam ID，呼叫 `POST /v1/api/unban` |

#### 使用提示

- **新建立的遠端連線**預設啟用線上地圖；既有連線可於首頁右鍵 **編輯遠端連線** 開啟「線上地圖」
- 遠端世界設定頁頂部會顯示唯讀提示；**無儲存按鈕**，修改請於遠端主機編輯 `PalWorldSettings.ini` 後重啟
- 封禁名單仍無法從 GUI 列出遠端 `banlist.txt`；若已知被封禁者的 UserId，可使用手動解封面板

#### Tier 2 仍不支援（留待 Tier 3）

| 功能 | 說明 |
|------|------|
| 遠端 INI 寫入 | 官方 REST 無設定寫入端點 |
| 封禁名單完整列舉 | 官方 REST 無 ban 列表 GET |
| SSH/SFTP 檔案存取 | 日誌、備份、Mod 等須 Tier 3 |
...
v1.4.0 已交付 **遠端伺服器管理 Tier 1（Phase 1–6）與 Tier 2（Phase 1–4）**；後續規劃（Tier 3 SSH/SFTP、Mod 相容檢查、設定匯入／匯出）見 [docs/ROADMAP_P3_FEATURES.md](/docs/ROADMAP_P3_FEATURES.md)。  
