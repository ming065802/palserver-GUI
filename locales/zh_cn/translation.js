...
const zh_cn = {
  ...
  RemoteBanListNotAvailable:
    '远程连接无法读取本地封禁名单；请在远程服务器上管理 banlist.txt，或通过 REST 封禁在线玩家。若已知 Steam ID，可在下方手动解除封禁。',
  RemoteUnbanSteamIdLabel: 'Steam ID / UserId',
  RemoteUnbanSteamIdPlaceholder: '例：76561198XXXXXXXX 或 steam_76561198XXXXXXXX',
  RemoteUnbanSuccess: '已发送解除封禁请求。',
  RemoteUnbanFailed: '解除封禁失败，请确认 Steam ID 与 Admin 密码是否正确。',
  RemoteUnbanAuthFailed: 'Admin 密码错误，无法解除封禁。',
  RemoteUnbanEmptyId: '请输入 Steam ID。',
  ...
  RemoteConnectionInvalidHost: '请输入远程主机 IP 或域名。',
  RemoteConnectionAuthFailed: 'Admin 密码错误，请确认远程服务器设置。',
  RemoteConnectionFailed: '无法连接至远程 REST API，请检查防火墙、端口转发与主机地址。',
  RemoteConnectionRestDisabled: '远程服务器未启用 REST API 或端点无法访问。',
  RemoteConnectionTestRequired: '建立前请先按「测试连接」并确认成功。',
  ...
  ScheduledStop: '定时关闭',
  ScheduledStopDesc:
    '每天在指定时间优雅关闭服务器（先存档再关闭）。需启用 REST API（或 RCON）。',
  ScheduledStopRemoteDesc:
    '每天在指定时间通过 REST API 优雅关闭远程服务器。',
  ScheduledStart: '定时启动',
  ScheduledStartDesc:
    '每天在指定时间自动启动本地服务器。仅在 palserver GUI 运行中时生效。',
  ScheduledRestart: '定时重启',
  ScheduledRestartDesc:
    '每天在指定时间关闭并重新启动本地服务器。需启用 REST API（或 RCON）。',
  ScheduledRestartRemoteDesc:
    '每天在指定时间通过 REST API 关闭远程服务器。远程服务器无法由 GUI 自动重新启动。',
  ScheduleWarningMinutes: '关闭前倒数（分钟）',
  ScheduleRequiresGuiOpen:
    '定时任务仅在 palserver GUI 运行中时生效；关闭 GUI 后排程不会执行。',
  ScheduleRequiresRestApi:
    '定时关闭与重启需启用 REST API（建议）或 RCON，并在服务器设定中设定管理员密码。',
  CrashRestart: '崩溃重启',
  CrashRestartDesc:
